...
class p2pChatCentralClient(threading.Thread):  # CENTRAL_CLIENT THREAD
    ...
    def registerToTheSystem(self):  # REGISTER OPERATION

        # send register message to the central server
        while True:
            ...
            # concat username and password with comma
            ...
            match loginStatus:
                ...
                case "success":
                    ...
                    print("In this application, you can "
                          ...
                          "******************************************************************************\n")
                    global join  # todo: i really dont care why join
                    join = True
                    ...
                    while True:
                        ...
                        match choice:
                                    ...
                                    match addUser:
                                        case "1":
                                            ...
                                            print("User successfully added in your chat list.\n"
                                                  ...
                                                  "you can select 2 in the main menu.\n")
                                            break
                                        case "2":
                                            break
                                        case _:
                            ...
                            case "2":  # chat
                                '''
                                activity_choice = input("1 wait, 2 send: ")
                                ...
                                if answer_message == "reject":
                                    print("user rejected chat request")
                                    pass
                                elif answer_message == "accept":

                                    pass
                                    '''
                                    thread = threading.Thread(target=self.start_listening_user)
                                    self.thread_2 = thread
                                    thread.start()
                                    '''

                                '''
                                peer2_ip, peer2_port = peer_addr.split(",")
                                self.peer2_conn = (peer2_ip, int(peer2_port))
                                time.sleep(1)
                                '''


                                # we may try to connect user directly with its ip address and port number.
                                '''
                                data = "chat req" +user_selected

                                socket.send(data.encode(FORMAT))
                                '''

                                '''
                                self.userStatus = True  # i really hate how this guy named things

                                if not self.userStatus:
                                    break
                                if len(self.conversationList) == 0:
                                    print("There is no one in the chat list, you need to add at least one user!\n")
                                else:
                                    global admin
                                    admin = True
                                    # for user in self.conversationList:
                                    #     chat_client = Chat_Client(str(user[0]),
                                    #                               int(user[1]), "LobyLeader")
                                    #     chat_client.start()
    ...
    def handle_server(self, server, nothing):
        ...
        '''
        while True:
            input_msg = input(f"{peer_name} want to connect with you. accept(1), reject(2): ")
            if input_msg == "2":
                conn.send("reject".encode("utf-8"))
                if input("wanna quit waiting 1(yes)/2(no)") == "1":
                    return
                self.handle_server(server, nothing)
                return
            elif input_msg == "1":
                conn.send("accept".encode("utf-8"))
                print("if you want to quit, you can write -1")
                msg = ""
                while True and msg != "-1":
                    msg = conn.recv(1024).decode("utf-8")
                    print("["+peer_name+"]" + msg)
                break
            else:
                print("please enter a valid input. such as '1' or '2'")
        '''
